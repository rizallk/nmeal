...
<div class="tambah-makanan">
  ...
  <form id="form-register" action="<?= site_url('tambah-makanan/register') ?>" method="post">
    <?= csrf_field() ?>
    <div class="row">
      <div class="col-md-6">
        <div class="mb-3">
  ...
  <script>
    ...
    const oldIngredientIds = <?= json_encode(old('ingredients') ?? []) ?>;
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (oldIngredientIds.length > 0) {
        oldIngredientIds.forEach(id => {
          const option = selectIngredient.querySelector(`option[value="${id}"]`);

          if (option) {
            const text = option.text;
            addIngredientToUi(id.toString(), text);
          }
        });
      }
