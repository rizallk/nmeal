...
<div class="tambah-alergen">
  ...
  <form id="form-register" action="<?= site_url('tambah-alergen/register') ?>" method="post">
    <?= csrf_field() ?>
    <div class="row">
      <div class="mb-3">
        <label for="name" class="form-label">Nama Alergen<span class="text-danger">*</span></label>
        <input type="text" name="name" class="form-control <?= $errors && isset($errors['name']) ? 'is-invalid' : '' ?>" id="name" placeholder="Nama Alergen" value="<?= old('name') ?>" required>
